...
public class Client1 {
  ...
  try{
    ...
    Socket socket1=new Socket("dc10.utdallas.edu",serverports[hash_fn1]); //  Connection to server H(o)
    Socket socket2=new Socket("dc10.utdallas.edu",serverports[hash_fn2]); //  connection to server H(o)+1 %7
    Socket socket3=new Socket("dc10.utdallas.edu",serverports[hash_fn3]); //  connection to server H(o)+2 %7
    ...
    //outStream3.writeUTF(clientMessage);       // Disrupted connection between client and server 
    //outStream3.flush();
    ...
    if (Server_Message.equals("Server Acknowledgement")){
      System.out.println(">> Client 1 recieves a successful ack from server " + from);
      }
    else 
    {
