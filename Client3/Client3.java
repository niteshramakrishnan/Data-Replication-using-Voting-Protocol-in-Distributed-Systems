...
public class Client3 {
  ...
  try{
    ...
    Socket socket1=new Socket("dc12.utdallas.edu",serverports[hash_fn1]); //  Connection to server H(o)
    Socket socket2=new Socket("dc12.utdallas.edu",serverports[hash_fn2]); //  connection to server H(o)+1 %7
    Socket socket3=new Socket("dc12.utdallas.edu",serverports[hash_fn3]); //  connection to server H(o)+2 %7
    ...
    if (Server_Message.equals("Server Acknowledgement")){
      System.out.println(">> Client 3 recieves a successful ack from server " + from);
      }
    else 
    {
