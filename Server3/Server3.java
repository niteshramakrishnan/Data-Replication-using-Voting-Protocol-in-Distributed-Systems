...
public class Server3 {
  ...
  public static void main(String[] args) throws Exception {
    try{
      ...
      ServerSocket server=new ServerSocket(9993);      // dc01.utdallas.edu 
...
class ServerClientThread extends Thread {
  ...
  String server_to_client_msg = Server3.server_no + ",Server Acknowledgement";
  ...
  public void run(){
    ...
    try{
      ...
      if(msg_no == 200)
      {
        System.out.println(">> Server 3 recieves: "+ "Server " + from_client + " request with message id:" + msg_no);
        
      }
      else if(msg_no == 100)
      {
        
        System.out.println(">> Server 3 recieves: "+ "Client " + from_client + " request with message id:" + msg_no);
      }     
      
      String server_msg = "200,"+ Server3.server_no + ",Server Request,"+hash1+","+hash2+","+hash3+","+"Vote Request";

      // trying to connect to other servers

      if(msg_no == 100)
      {
        Server3.if_msg_recieved = true;
        
        if(Server3.server_no == hash1 + 1)
        {
          try
          {
      ...
      else if(msg_no == 200)
      {
        ...
        if(Server3.if_msg_recieved)
        {
          //System.out.println(Server3.if_msg_recieved);
          outStream.writeUTF(server_to_client_msg);
          outStream.flush();
        }
        else 
        {
          outStream.writeUTF(Server3.server_no+",No Acknowledgement");
          outStream.flush();
        }
      }
      else if (msg_no == 0)
      {
        Server3.if_msg_recieved = false;
        outStream.writeUTF(Server3.server_no+",No Acknowledgement");
        outStream.flush();
