...
public class Server6 {
  ...
  public static void main(String[] args) throws Exception {
    try{
      ...
      ServerSocket server=new ServerSocket(9996);      // dc01.utdallas.edu 
      
      
      
      System.out.println(">> Server 6 Started ");
      // Files creation
      try {
...
class ServerClientThread extends Thread {
  ...
  String server_to_client_msg = Server6.server_no + ",Server Acknowledgement";
  ...
  public void run(){
    ...
    try{
      ...
      if(msg_no == 200)
      {
        System.out.println(">> Server 6 recieves: "+ "Server " + from_client + " request with message id:" + msg_no);
        
      }
      else if(msg_no == 100)
      {
        
        System.out.println(">> Server 6 recieves: "+ "Client " + from_client + " request with message id:" + msg_no);
      }     
      
      String server_msg = "200,"+ Server6.server_no + ",Server Request,"+hash1+","+hash2+","+hash3+","+"Vote Request";

      // trying to connect to other servers

      if(msg_no == 100)
      {
        Server6.if_msg_recieved = true;
        
        if(Server6.server_no == hash1 + 1)
        {
          try
          {
      ...
      else if(msg_no == 200)
      {
        ...
        if(Server6.if_msg_recieved)
        {
          //System.out.println(Server6.if_msg_recieved);
          outStream.writeUTF(server_to_client_msg);
          outStream.flush();
        }
        else 
        {
          outStream.writeUTF(Server6.server_no+",No Acknowledgement");
          outStream.flush();
        }
      }
      else if (msg_no == 0)
      {
        Server6.if_msg_recieved = false;
        outStream.writeUTF(Server6.server_no+",No Acknowledgement");
        outStream.flush();
