...
public class Client5 {
  ...
  try{
    ...
    Socket socket1=new Socket("dc14.utdallas.edu",serverports[hash_fn1]); //  Connection to server H(o)
    Socket socket2=new Socket("dc14.utdallas.edu",serverports[hash_fn2]); //  connection to server H(o)+1 %7
    Socket socket3=new Socket("dc14.utdallas.edu",serverports[hash_fn3]); //  connection to server H(o)+2 %7
    ...
    //outStream2.writeUTF(clientMessage);         // Disrupted connection between client and server 
    //outStream2.flush();
    ...
    if (Server_Message.equals("Server Acknowledgement")){
      System.out.println(">> Client 5 recieves a successful ack from server " + from);
      }
    else 
    {
